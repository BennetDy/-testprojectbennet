...
class C(BaseConstants):
    NAME_IN_URL = 'real_efforts_number'
    PLAYERS_PER_GROUP = None
    NUM_ROUNDS = 2
    payment_per_correct = cu(1)
...
class AddNumbers(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        if player.sum_of_numbers == player.number_entered:
            player.payoff = C.payment_per_correct
...
page_sequence = [AddNumbers, Result, CombinedResults]
